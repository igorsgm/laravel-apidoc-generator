...
return [
        ...
        'event' => [
            [
                ...
                "script" => [
                    ...
                    "exec" => [
                        "// Set true if the Cookie that you added at STREAMLABS_COOKIE was taken from the account owner. Set false if taken while acting like him",
                        "const isUserOwner = true;",
                        "",
                        "// Add the flags that the account owner his on _user_flags table (You can check it on Admin panel)",
                        "// The possible values are: 'refund-prime', 'prime-benefits', 'admin', 'collectibles', 'marketplace', 'payments-admin', 'isMerchAdmin'",
                        "const userFlags = [];",
                        "",
                        "// Some URIs that will be ignored for the tests.",
                        "const pathsToIgnore = [",
                        "  // webhook specific",
                        "  '/actblue/webhooks',",
                        "  '/api/v5/merchandise/printify-webhook',",
                        "  '/api/v5/slobs/media/upload', // ignoring upload tests",
                        "  '/paypal/auth', // needs vip flag",
                        "  '/paypal/connect', // needs vip flag",
                        "  // ignore repeat alerts tests because they require body params",
                        "  '/api/v5/slobs/widget/repeatalert',",
                        "  '/api/v5/widget/repeatalert',",
                        "  '/api/v5/slobs_platform/alerts/repeat',",
                        "  '/api/v5/pro/user', // ignore routes with 'shadow-user' middleware",
                        "  '/api/v5/charity/tvg5/export', // must be user_id = '4902133'",
                        "  '/api/v5/charity/export/' + pm.environment.get('CHARITY_CAMPAIGN_ID'),",
                        "];",
                        "",
                        "let route = JSON.parse(pm.request.description.content);",
                        "let middlewares = route.middlewares;",
                        "let roleGroup = pm.globals.get('role');",
                        "",
                        "let isAdminOnlyRoute = function () {",
                        "  if (!middlewares) {",
                        "    return false;",
                        "  }",
                        "",
                        "  return middlewares.some((item) =>",
                        "      ['admin', 'flag:payments-admin', 'admin.user.action'].includes(item)) ||",
                        "    (route.role && route.role.includes('admin') && !route.role.includes('owner'))",
                        "}();",
                        "",
                        "let routeFlagMiddlewares = function () {",
                        "  if (!middlewares) {",
                        "    return [];",
                        "  }",
                        "",
                        "  let flagMiddlewaresList = ['refund-prime', 'prime-benefits', 'admin', 'collectibles', 'marketplace', 'payments-admin', 'crates.admin'];",
                        "  return flagMiddlewaresList.filter((flagName) =>",
                        "    middlewares.some((item) => [flagName, 'flag:' + flagName, 'flag:' + flagName + ',true'].includes(item)),",
                        "  );",
                        "}();",
                        "",
                        "let roleString = function () {",
                        "  if (isAdminOnlyRoute) {",
                        "    return 'Users with Admin Flag only';",
                        "  }",
                        "",
                        "  if (routeFlagMiddlewares.length > 0) {",
                        "    return \"Users with flag: '\" + routeFlagMiddlewares[0] + \"'\";",
                        "  }",
                        "",
                        "  if (isUserOwner) {",
                        "    return 'Owner user';",
                        "  }",
                        "",
                        "  return 'Users with role: ' + roleGroup;",
                        "}();",
                        "",
                        "let isToSkipTest = function () {",
                        "  let path = pm.request.url.getPath();",
                        "",
                        "  // Skip tests for ignored paths + giveaway + twitch extensions routes test",
                        "  if (pathsToIgnore.includes(path) || path.includes('api/v5/giveaway') || path.includes('/twitch-extensions')) {",
                        "    return true;",
                        "  }",
                        "",
                        "  // Skip tests for json responses with errors (because it means that you could access the endpoint)",
                        "  if (pm.response.contentInfo().mimeFormat === 'json') {",
                        "    let jsonResponse = pm.response.json();",
                        "",
                        "    // true if jsonResponse contains at least one of the following properties",
                        "    return Object.keys(jsonResponse).some(property => ['error', 'status', 'message', 'success'].includes(property));",
                        "  }",
                        "",
                        "  return false;",
                        "}();",
                        ...
                        "  if (",
                        "    (isAdminOnlyRoute && !userFlags.includes('admin')) ||",
                        "    (routeFlagMiddlewares.length > 0 && !userFlags.includes(routeFlagMiddlewares[0])) ||",
                        "    (middlewares && middlewares.includes('merch-admin') && !userFlags.includes('isMerchAdmin'))",
                        "  ) {",
                        ...
                        "",
                        "if (!isToSkipTest) {",
                        "  let testName = 'Route can be accessed by: ' + roleString;",
                        "  pm.test(testName, accessFunc);",
                        "}"
                    ]
                ]
            ]
    ...
    'strategies' => [
        ...
        'headers' => [
            ...
            \Mpociot\ApiDoc\Extracting\Strategies\RequestHeaders\CustomHeaders::class,
        ],
        'bodyParameters' => [
